...
const EmailVerificationContent = () => {
  ...
  const verifyEmail = async () => {
    try {
      ...
      const response = await axios.get(`${devURL}/user/auth/verify-email?token=${token}`);
    ...
    <div className="email-verification-container">
      <div className="email-verification-card">
        <div className="email-verification-header">
          <h1 style={{ color: 'white' }}>DigiCare4u</h1>
          <p style={{ color: 'white' }}>Track and manage your devices effortlessly and securely</p>
        </div>

        <div className="email-verification-content">
          <h2>Welcome, {userData?.name || "User"}!</h2>
          <h3>You have been added by {userData?.superAdminName || "DigiCare4u"}.</h3>
          <p>
